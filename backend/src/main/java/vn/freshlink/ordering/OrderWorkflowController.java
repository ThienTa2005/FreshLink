...
public class OrderWorkflowController {
    ...
    @PutMapping("/{id}") @Transactional public ApiResponse<?> update(@AuthenticationPrincipal Actor a,@PathVariable long id,@Valid @RequestBody Update r){
        var o=lock(id);long org=((Number)o.get("restaurant_id")).longValue();a.requireOrganization(org,"RESTAURANT_MANAGER","RESTAURANT_PURCHASER");cutoff((LocalDate)o.get("delivery_date"));cutoff(r.date());
        if(!Set.of("SUBMITTED","CONFIRMED").contains(o.get("order_status")))throw new ResponseStatusException(HttpStatus.CONFLICT,"Đơn đã vào xử lý và không thể sửa");
    ...
    @PostMapping("/{id}/submit-approval") @Transactional public ApiResponse<?> submit(@AuthenticationPrincipal Actor a,@PathVariable long id){var o=lock(id);a.requireOrganization(((Number)o.get("restaurant_id")).longValue(),"RESTAURANT_PURCHASER");cutoff((LocalDate)o.get("delivery_date"));if(!"CONFIRMED".equals(o.get("order_status"))||!"NOT_REQUIRED".equals(o.get("approval_status")))throw new ResponseStatusException(HttpStatus.CONFLICT,"Đơn không thể gửi duyệt");jdbc.update("UPDATE customer_orders SET order_status='SUBMITTED',approval_status='PENDING',confirmed_at=NULL WHERE order_id=?",id);audit(a,"SUBMIT_ORDER_APPROVAL",id,"pending");return ApiResponse.success(id,"Đã gửi quản lý duyệt");}
    @PostMapping("/{id}/approve") @Transactional public ApiResponse<?> approve(@AuthenticationPrincipal Actor a,@PathVariable long id,@Valid @RequestBody Decision r){var o=lock(id);a.requireOrganization(((Number)o.get("restaurant_id")).longValue(),"RESTAURANT_MANAGER");cutoff((LocalDate)o.get("delivery_date"));if(!"PENDING".equals(o.get("approval_status"))||!"SUBMITTED".equals(o.get("order_status")))throw new ResponseStatusException(HttpStatus.CONFLICT,"Đơn không chờ duyệt");if(!r.approved()&&(r.reason()==null||r.reason().isBlank()))throw new IllegalArgumentException("Cần lý do từ chối");String s=r.approved()?"CONFIRMED":"CANCELLED";jdbc.update("UPDATE customer_orders SET order_status=?,approval_status=?,approved_by=?,approved_at=UTC_TIMESTAMP(3),rejection_reason=?,confirmed_at=IF(?='CONFIRMED',UTC_TIMESTAMP(3),NULL),cancelled_at=IF(?='CANCELLED',UTC_TIMESTAMP(3),NULL) WHERE order_id=?",s,r.approved()?"APPROVED":"REJECTED",a.userId(),r.reason(),s,s,id);jdbc.update("INSERT INTO order_status_history(order_id,old_status,new_status,reason,changed_by) VALUES (?,'SUBMITTED',?,?,?)",id,s,r.reason(),a.userId());audit(a,r.approved()?"APPROVE_ORDER":"REJECT_ORDER",id,r.reason());return ApiResponse.success(id,r.approved()?"Đã duyệt đơn":"Đã từ chối đơn");}
    @PostMapping("/{id}/cancel") @Transactional public ApiResponse<?> cancel(@AuthenticationPrincipal Actor a,@PathVariable long id,@Valid @RequestBody Reason r){var o=lock(id);a.requireOrganization(((Number)o.get("restaurant_id")).longValue(),"RESTAURANT_MANAGER","RESTAURANT_PURCHASER");cutoff((LocalDate)o.get("delivery_date"));if(!Set.of("SUBMITTED","CONFIRMED","SOURCING").contains(o.get("order_status")))throw new ResponseStatusException(HttpStatus.CONFLICT,"Đơn không thể hủy");var xs=jdbc.queryForList("SELECT ba.batch_allocation_id,ba.batch_id,ba.allocated_quantity FROM batch_allocations ba JOIN order_items i ON i.order_item_id=ba.order_item_id WHERE i.order_id=? AND ba.allocation_status='RESERVED' FOR UPDATE",id);for(var x:xs){jdbc.update("UPDATE batches SET allocated_quantity=allocated_quantity-? WHERE batch_id=?",x.get("allocated_quantity"),x.get("batch_id"));jdbc.update("UPDATE batch_allocations SET allocation_status='CANCELLED' WHERE batch_allocation_id=?",x.get("batch_allocation_id"));}jdbc.update("UPDATE customer_orders SET order_status='CANCELLED',cancelled_at=UTC_TIMESTAMP(3),cancellation_reason=? WHERE order_id=?",r.reason(),id);jdbc.update("UPDATE order_items SET item_status='CANCELLED' WHERE order_id=?",id);audit(a,"CANCEL_ORDER",id,r.reason());return ApiResponse.success(id,"Đã hủy đơn và giải phóng lượng giữ");}
}
