...
export const FreshLinkMap: React.FC<FreshLinkMapProps> = ({
  ...
  const [mapLayerType, setMapLayerType] = useState<'VOYAGER' | 'OSM'>('VOYAGER')
      ...
      {showToolbar && (
        <div className="freshlink-map-header">
          ...
          <div className="freshlink-map-toolbar">
            ...
            <button
              className="map-action-btn"
              title={`Chuyển lớp bản đồ (${mapLayerType === 'VOYAGER' ? 'Logistics Voyager' : 'OpenStreetMap'})`}
              onClick={() => setMapLayerType(mapLayerType === 'VOYAGER' ? 'OSM' : 'VOYAGER')}
            >
              <span className="material-symbols-outlined" style={{ fontSize: 16 }}>
                layers
              </span>
